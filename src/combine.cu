    ...
    int fn_id)
{
  ...
  // TODO: add block stride
  // if(out_pos < out_size) {
  //   to_index(out_pos, out_shape, out_index, shape_size);
  //   int ori_pos = index_to_position(out_index, out_strides, shape_size);
  //   for(int i = ori_pos; i < a_shape[reduce_dim]; i += a_strides[reduce_dim]){
  //     reduce_value = fn(fn_id, a_storage[i], reduce_value);
  // }
  ...
  // Hard encoded to 1024 due to the max thread number is 1024
  ...
  if(out_pos < out_size){
    // Block stride 
    for(; out_pos < out_size; out_pos += gridDim.x){
      to_index(out_pos, out_shape, out_index, shape_size);
      int ori_start = index_to_position(out_index, a_strides, shape_size);

      // Thread stride
      float val = reduce_value;
      for(int i = tid; i < reduce_len; i += blockDim.x){
        int ori_pos = ori_start + i * a_strides[reduce_dim];
        val = fn(fn_id, a_storage[ori_pos], val);
      }
      ...
      // Tree reduction
      for(int step = 1; step < reduce_len; step *= 2){
        if(tid % (step * 2) == 0){
          sdata[tid] = fn(fn_id, sdata[tid], sdata[tid + step]);
        }
        __syncthreads();
      }
    ...
    const int *b_strides)
{
  ...
  /* Version 1.0: A block for a matrix without tile*/
  ...
  // for (int tid = idx; tid < total; tid += blockDim.x * blockDim.y) {
  //   int row = tid / p;
  //   int col = tid % p;
  ...
  int m = out_shape[1]; int p = out_shape[2];
  int width = a_shape[2];
  ...
  int max_steps = ( width + TILE -1 ) / TILE;

  for(int phase = 0; phase < max_steps; ++phase){
    int a_col = phase * TILE + tx;
    if (row < m && a_col < width) {
      int a_idx[3] = {batch, row, a_col};
      a_shared[ty][tx] = a_storage[index_to_position(a_idx, a_strides, 3)];
    } else {
      a_shared[ty][tx] = 0.0f;
    }

    int b_row = phase * TILE + ty;
    if(col < p && b_row < width){
      int b_idx[3] = {batch, b_row, col};
      b_shared[ty][tx] = b_storage[index_to_position(b_idx, b_strides, 3)];
    } else {
      b_shared[ty][tx] = 0.0f;
    }
...
extern "C"
{
      ...
      int batch, int m, int p)
  {
    ...
    //dim3 gridDims(1, 1, batch); // for Version 1.0
